...
void VirtualWorldCycle()
{
	...
	{                              // nale¿y na nowo obliczyæ œredni czas cyklu fDt
		...
		sprintf(text, "WZR-2019/20, tem.2, wer.i (jak Idzi), czêstoœæ pr.wys. = %0.2f[r/s]  œr.odl = %0.3f[m]  œr.ro¿n.k¹t. = %0.3f[st]",
	...
	MovableObject *car = (network_vehicles.size() > 0 ? network_vehicles[my_vehicle->iID] : NULL);
	if (car != NULL)
	{
	...
	for (map<int, MovableObject*>::iterator it = network_vehicles.begin(); it != network_vehicles.end(); ++it)
	{
		...
		//veh->state.qOrient = ....
		veh->state.qOrient = AsixToQuat((veh->state.vV_ang*fDt + veh->state.vA_ang*fDt*fDt / 2.0f),
			(veh->state.vV_ang*fDt + veh->state.vA_ang*fDt*fDt / 2.0f).length()) * veh->state.qOrient;
	...
	int       nCmdShow)
{
	...
	window_handle = CreateWindow(class_name, "WWC/WZR-lab 2019/20 temat 2 - Nawigacja obliczeniowa - wersja i (jak Idzi)", WS_OVERLAPPEDWINDOW | WS_VISIBLE | WS_CLIPCHILDREN | WS_CLIPSIBLINGS,
		500, 50, 900, 900, NULL, NULL, hInstance, NULL);
