...
void DrawScene()
{
	...
	GLfloat GroundSurface[] = { 0.7*(0.8 + 0.4*sin_angminpi), 0.5*(0.2 + 0.8*sin_angminpi), 0.1*(0.1 + 0.9*sin_angminpi), 1.0f };
	...
	for (int w = -1; w < 2; w++)
		for (int k = -1; k < 2; k++)
		{
			...
			//for each (auto&& it in network_vehicles) it.second->DrawObject();   // od VC 2013 (C++ 11)
