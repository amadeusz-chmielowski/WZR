...
MovableObject::MovableObject()             // konstruktor                   
{
	...
	// sta³e samochodu
	mass_own = 24;// 8.0 + 8.0*(float)rand() / RAND_MAX;			// masa obiektu [kg]
	...
	steer_wheel_ret_speed = 0.3; // prêdkoœæ powrotu kierownicy w rad/s (gdy zostateie puszczona)
	...
	state.vPos.x = -50;
	state.vPos.z = 79;
	quaternion qObr = AsixToQuat(Vector3(0, 1, 0), -0.6*PI / 180.0); // obrót obiektu o k¹t 30 stopni wzglêdem osi y:
