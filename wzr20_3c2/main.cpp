...
#pragma region auction
bool in_auction = false;
vector<int> iIDs_bidding;
#define AUCTION_TIMEOUT 60 //seconds
int auction_timeout = 0;
#define AUCTION_BREAK 20 //seconds
...
DWORD WINAPI ReceiveThreadFunction(void *ptr)
{
	...
	while (1)
	{
		...
		case AUCTION_STARTER:
		{
			if (frame.iID == my_vehicle->iID) {
				//sam siebie nie moge zaprosic
				break;
			}
			else if (frame.iID_receiver != my_vehicle->iID) {
				//aukcja nie wyslana do mnie
				break;
			}
			else {
				string msg = "";
				msg += "Invite to auction from " + to_string(frame.iID);
				LPCSTR message = msg.c_str();
	...
	int       nCmdShow)
{
	...
	main_window = CreateWindow(class_name, "WZR 2019/20, temat 3, wersja c [Y-autopilot, F10-test autop.]", WS_OVERLAPPEDWINDOW | WS_VISIBLE | WS_CLIPCHILDREN | WS_CLIPSIBLINGS,
...
void MessagesHandling(UINT message_type, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
	{
		...
		case 'B':
		{
			if (!in_auction && time_between_auctions == 0) {
				Frame frame;
				frame.frame_type = AUCTION_STARTER;
				frame.iID = my_vehicle->iID;
				float min_dist = 100;
				for (map<int, MovableObject*>::iterator it = network_vehicles.begin(); it != network_vehicles.end(); ++it)
				{
					if (it->second)
					{
						MovableObject *ob = it->second;
						if (min_dist >= (ob->state.vPos - my_vehicle->state.vPos).length()) {
							if (ob->party_number == my_vehicle->party_number) {
								frame.iID_receiver = ob->iID;
								int iRozmiar = multi_send->send((char*)&frame, sizeof(Frame));
								in_auction = true;
								timer.setInterval([&]() {
									time_between_auctions += 5;
								}, 5000);
								iIDs_bidding.clear();
								agrrement_values.clear();
							}
		...
		case '+':
		{
			if (in_auction) {
				...
				frame.iID = my_vehicle->iID;
				float min_dist = 100;
				frame.iID_receiver = iIDs_bidding[0];
